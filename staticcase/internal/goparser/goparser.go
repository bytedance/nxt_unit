...
func (p *Parser) parseFunctions(srcPath string, fset *token.FileSet, f *ast.File, fs []*ast.File) []*models.Function {
	...
	for _, d := range f.Decls {
		...
		funcInfo := parseFunc(fDecl, ul, el)
		opt := atgconstant.Options{
			FilePath: srcPath,
			FuncName: func() string {
				switch funcInfo.Receiver {
				case nil:
		...
		// We log the error and see why this will happen
		if err != nil {
			logextractor.ExecutionLog.DebugInfo(err.Error())
		}

		if funcInfoSSA != nil && err == nil {
			for i := 0; i < len(funcInfoSSA.Function.Params); i++ {
				j := i
				// function's first param is receiver of SSA
				...
				if j >= len(funcInfo.Parameters) {
					continue
				}
				switch funcInfoSSA.Function.Params[i].Type().Underlying().(type) {
				case *types.Interface:
					if funcInfo.Parameters[j].Type != nil && !funcInfo.Parameters[j].Type.IsInterface {
						funcInfo.Parameters[j].Type.IsInterface = true
					}
				}
...
func parseExpr(e ast.Expr, ul map[string]types.Type) *models.Expression {
	defaultFunc := func() *models.Expression {
	...
	case *ast.SelectorExpr:
		// it doesn't recognize the interface from other package by AST
		return defaultFunc()
		// rev, ok := v.X.(*ast.Ident)
		// if !ok {
		// 	return defaultFunc()
		// }
		// if v.Sel.Obj == nil {
		// 	return defaultFunc()
		// }
		// decl, ok := v.Sel.Obj.Decl.(*ast.TypeSpec)
		// if !ok {
		// 	return defaultFunc()
		// }
		...
		// 	return defaultFunc()
		// }
		// val := fmt.Sprintf("struct{%v.%v}", rev.Name, decl.Name)
		// return &models.Expression{
		// 	Value:       val,
		// 	Underlying:  underlying(val, ul),
		// 	IsInterface: true,
		// 	IsWriter:    val == "io.Writer",
		// }
	// case *ast.InterfaceType,we write a struct to cover interface
	case *ast.Ident:
		if v.Obj == nil {
			return defaultFunc()
		}
		decl, ok := v.Obj.Decl.(*ast.TypeSpec)
		if !ok {
			return defaultFunc()
		}
		_, ok = decl.Type.(*ast.InterfaceType)
		...
		val := fmt.Sprintf("*struct{%v;  InterfaceTag bool}", v.Name)
		return &models.Expression{
			Value:       val,
			Underlying:  underlying(val, ul),
			IsInterface: true,
